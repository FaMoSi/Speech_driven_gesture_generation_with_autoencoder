...
def train_CNN(model_file):
    ...
    X_train = X_train.reshape(X_train.shape[0], 11, 3, N_INPUT, 1)
    X_validation = X_validation.reshape(X_validation.shape[0], 12, 3, N_INPUT, 1)
    
    # Define Keras model
    model = Sequential()

    # CNN 
    model.add(TimeDistributed(Conv2D(36, (5, 5)), input_shape=(11, 3, N_INPUT, 1)))
